...
{
  if(Robot1.feature_selection.gyroscope_solving==AHRS)
  {
  
    osDelay(INS_TASK_INIT_TIME);//进入阻塞
    while(BMI088_init(&hspi1, 1))
    {
        osDelay(100);
    }
    while(ist8310_init())
    ...
    BMI088_read(bmi088_real_data.gyro, bmi088_real_data.accel, &bmi088_real_data.temp);//获取陀螺仪数据
    //旋转和零漂
    ...
    //SPI1_DMA初始化
    SPI1_DMA_init((uint32_t)gyro_dma_tx_buf, (uint32_t)accel_dma_tx_buf, SPI_DMA_GYRO_LENGHT);
    ...
    if(Robot1.feature_selection.gyroscope_solving == EKF)
    {
      INS_Init();
      while(1)
      {
...
{
    *yaw = atan2f(2.0f*(q[0]*q[3]+q[1]*q[2]), 2.0f*(q[0]*q[0]+q[1]*q[1])-1.0f)*57.3f;
    *pitch = asinf(-2.0f*(q[1]*q[3]-q[0]*q[2]))*57.3f;
    *roll = atan2f(2.0f*(q[0]*q[1]+q[2]*q[3]),2.0f*(q[0]*q[0]+q[3]*q[3])-1.0f)*57.3f;
}
